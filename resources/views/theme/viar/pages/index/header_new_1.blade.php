...
{{--						</li>--}}
				...
				<div class="vz-art header-button">
					<a href="{{ route('new_account.index') }}"
						class="acc__link vz-art header-user @if (!Auth::check()) js-popup-login @endif"
						aria-label="user link">
						<i class="fa-user"></i>
					</a>
					<a href="{{ route('cart.index') }}" rel="nofollow" class="vz-art header-cart @if (!Auth::check() && !session()->has('email')) js-popup-repair-basket @endif" aria-label="cart link">
						<i class="fa-cart"></i>
					</a>
				</div>
				...
				<div class="vz-art language _spollers _one _esc">
					<p class="_spoller ">
{{--                        <img class="lozad" loading="lazy" src="{{ asset('images/flag/' . app()->getLocale() . '.svg') }}"> --}}
                        {{ ucfirst(app()->getLocale()) }} <i
					...
					<ul class="ver1_langs">
						@foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
							@php
                                if (!in_array($localeCode, $allowed_lang)) { continue; }
                                 $cur_url = $localized_urls[$localeCode] ?? LaravelLocalization::getLocalizedURL($localeCode, null, [], true);
