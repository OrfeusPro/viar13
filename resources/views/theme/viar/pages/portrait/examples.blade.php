            ...
            <div class="before-after__block">
                ...
                <div class="ba-slider">
                    @if(!$ba_items->isEmpty())
                        <picture>
                            @php
                                $portraitExampleBeforeSources = image_picture_sources($ba_items[0]->getUrl(), true);
                            @endphp
                            @if(!empty($portraitExampleBeforeSources['src_webp']))
                                <source srcset="{{ $portraitExampleBeforeSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($portraitExampleBeforeSources['src']) && !empty($portraitExampleBeforeSources['type']))
                                <source srcset="{{ $portraitExampleBeforeSources['src'] }}" type="{{ $portraitExampleBeforeSources['type'] }}">
                            @endif
                            <img src="{{ $portraitExampleBeforeSources['src'] }}" @altAttrs($item, 'media:new_before_items', $ba_items[0]->getUrl(), null, $ba_items[0]->getCustomProperty('image_alt_' . app()->getLocale()), $ba_items[0]->getCustomProperty('image_title_' . app()->getLocale()))/>
                        </picture>
                    @endif
                    @if(!$ba_items_after->isEmpty())
                        <div class="resize">
                            <picture>
                                @php
                                    $portraitExampleAfterSources = image_picture_sources($ba_items_after[0]->getUrl(), true);
                                @endphp
                                @if(!empty($portraitExampleAfterSources['src_webp']))
                                    <source srcset="{{ $portraitExampleAfterSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($portraitExampleAfterSources['src']) && !empty($portraitExampleAfterSources['type']))
                                    <source srcset="{{ $portraitExampleAfterSources['src'] }}" type="{{ $portraitExampleAfterSources['type'] }}">
                                @endif
                                <img srcset="{{ $portraitExampleAfterSources['src'] }}" @altAttrs($item, 'media:new_after_items', $ba_items_after[0]->getUrl(), null, $ba_items_after[0]->getCustomProperty('image_alt_' . app()->getLocale()), $ba_items_after[0]->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endif
                                ...
                                <a class="examples-slide__photo">
                                        <picture>
											@if(isset($ba_items2[$loop->index]) && $ba_items2[$loop->index]->getUrl())
                                                @php
                                                    $portraitExampleWorkSources = image_picture_sources($ba_item->getUrl(), true);
                                                @endphp
                                                @if(!empty($portraitExampleWorkSources['src_webp']))
                                                    <source srcset="{{ $portraitExampleWorkSources['src_webp'] }}" type="image/webp">
                                                @endif
                                                @if(!empty($portraitExampleWorkSources['src']) && !empty($portraitExampleWorkSources['type']))
                                                    <source srcset="{{ $portraitExampleWorkSources['src'] }}" type="{{ $portraitExampleWorkSources['type'] }}">
                                                @endif
                                            <img src="{{ $portraitExampleWorkSources['src'] }}" data-fancybox="group" data-src="{{ $ba_items2[$loop->index]->getUrl() }}" @altAttrs($item, 'media:works_examples_new', $ba_item->getUrl(), null, $ba_item->getCustomProperty('image_alt_' . app()->getLocale()), $ba_item->getCustomProperty('image_title_' . app()->getLocale())) loading="lazy"/>
											@endif
                                        </picture>
                                </a>
