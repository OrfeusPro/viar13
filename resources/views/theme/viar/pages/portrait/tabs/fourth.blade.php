<div class="about__block about__fourth  brief-v @if($is_hidden ?? true) hidden-block @endif">
                    ...
                    <div class="examples-slide">
                        <a href="{{ $s_item->getUrl() }}" class="examples-slide__photo"
                           data-fancybox="examples" aria-label="">
                            <picture>
                                @php
                                    $ourWorkImageSources = image_picture_sources($s_item->getUrl(), true);
                                @endphp
                                @if(!empty($ourWorkImageSources['src_webp']))
                                    <source srcset="{{ $ourWorkImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($ourWorkImageSources['src']) && !empty($ourWorkImageSources['type']))
                                    <source srcset="{{ $ourWorkImageSources['src'] }}" type="{{ $ourWorkImageSources['type'] }}">
                                @endif
                                <img src="{{ $ourWorkImageSources['src'] }}" @altAttrs($item, 'media:our_works_new', $s_item->getUrl(), null, $s_item->getCustomProperty('image_alt_' . app()->getLocale()), $s_item->getCustomProperty('image_title_' . app()->getLocale())) loading="lazy"/>
                            </picture>
                        </a>
