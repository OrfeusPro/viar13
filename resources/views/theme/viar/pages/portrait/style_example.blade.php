<section class="portrait-list">
	<div class="section-frame">
		<div class="section-inner">
			<div class="section-tabs">
				<div class="section-tabs__inner">
					<div class="section-tabs__item tabs-js active">{!! trans('portrait_royal.s_reason_cat_all') !!}</div>
					@foreach ($list['list_category'] as $cat)
						<div class="section-tabs__item tabs-js" data-cat="{{ trim($cat) }}">@lang('portrait_royal.' . trim($cat))</div>
					@endforeach
				</div>
			</div>
			<div class="portrait-list__inner portrait-list__slider">
				@foreach ($list['list'] as $item)
					<div class="portrait-list__item" data-cat="{{ $item->category }}">
						<p>{{ $item->title }}</p>
						{{-- <p>@if(isset($item->title) && $item->title) {{ $item->title }} @else @lang('portrait_royal.' . trim($item->category)) #{{ $loop->index+100 }} @endif</p> --}}
