<section class="service-info section-block">
    <div class="section-frame">
        <div class="section-inner">
            <div class="sectionDefault-tabs">
                <div class="sectionDefault-tabs__item active">{{trans('portrait_royal.s_service_tab_basic')}}</div>
                <div class="sectionDefault-tabs__item       ">{{trans('portrait_royal.s_service_tab_extra')}}</div>
            </div>
            <div class="service-info__content">
                <div class="service-info__block">
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service1Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service1.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service1.webp') }}" alt="">
                            </picture>
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service2Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service2.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service2.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_1')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_1_2')}}</p>
                            </div>
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_2')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_2_2')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service3Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service3.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service3.webp') }}" alt="">
                            </picture>
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service4Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service4.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service4.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_3')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_3_2')}}</p>
                            </div>
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_4')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_4_2')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service5Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service5.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service5.webp') }}" alt="">
                            </picture>
                            <picture>
                                <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service6Min.webp') }}" type="image/webp">
                                <source srcset="{{ ver_asset('images/sharj/service6.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service6.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_5')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_5_2')}}</p>
                            </div>
                            <div class="content-item">
                                <div class="p name">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="10" cy="10" r="10" fill="white"></circle>
                                        <circle cx="10" cy="10" r="10" fill="#02BC4D"></circle>
                                        <g clip-path="url(#clip0_1534_1841)">
                                            <path d="M9.08659 14.3879C8.86917 14.3879 8.65925 14.3061 8.49868 14.1586L5.28114 11.2022C4.92752 10.8773 4.90441 10.3276 5.22928 9.97331C5.55479 9.62094 6.10396 9.5972 6.4582 9.92146L9.03036 12.2856L13.4856 7.36929C13.8079 7.01317 14.3584 6.98631 14.7145 7.30869C15.07 7.63107 15.0975 8.18086 14.7745 8.53698L9.73135 14.103C9.57515 14.2742 9.35836 14.3767 9.12657 14.3879C9.11283 14.3879 9.09971 14.3879 9.08659 14.3879Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_1534_1841">
                                                <rect width="10" height="7.30476" fill="white" transform="translate(5 7.08301)"></rect>
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <p>{{trans('portrait_royal.s_service_basic_list_6')}}</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_basic_list_6_2')}}</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="service-info__block hidden-block">
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service1Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service7.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service7.webp') }}" alt="">
                            </picture>
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service2Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service8.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service8.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-text">
                                <div class="title">
                                    <p>{{trans('portrait_royal.s_service_extra_list_1')}}</p>
                                    <p>от 30 €</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_extra_list_1_2')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service3Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service9.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service9.webp') }}" alt="">
                            </picture>
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service4Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service10.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service10.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-text">
                                <div class="title">
                                    <p>{{trans('portrait_royal.s_service_extra_list_2')}}</p>
                                    <p>5 €</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_extra_list_2_2')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="service-info__row">
                        <div class="image-group">
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service5Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service11.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service11.webp') }}" alt="">
                            </picture>
                            <picture>
                                <!-- <source media="(max-width: 576px)" srcset="{{ ver_asset('images/sharj/service6Min.webp') }}" type="image/webp"> -->
                                <source srcset="{{ ver_asset('images/sharj/service12.webp') }}" type="image/webp">
                                <img width="315" height="275" src="{{ ver_asset('images/sharj/service12.webp') }}" alt="">
                            </picture>
                        </div>
                        <div class="content">
                            <div class="content-text">
                                <div class="title">
                                    <p>{{trans('portrait_royal.s_service_extra_list_3')}}</p>
                                    <p>5 €</p>
                                </div>
                                <p>{{trans('portrait_royal.s_service_extra_list_3_2')}}</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="service-info__bottom">
                <p>{!!  trans('portrait_royal.s_service_note') !!}</p>
                <svg width="111" height="84" viewBox="0 0 111 84" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0.378491 74.9313C-0.0533335 75.2756 -0.12428 75.9048 0.220024 76.3366L5.83082 83.3736C6.17512 83.8054 6.8043 83.8763 7.23612 83.532C7.66794 83.1877 7.73889 82.5585 7.39459 82.1267L2.40722 75.8716L8.66231 70.8843C9.09413 70.54 9.16508 69.9108 8.82077 69.479C8.47647 69.0471 7.84729 68.9762 7.41547 69.3205L0.378491 74.9313ZM109.5 1C108.504 1.08757 108.504 1.08727 108.504 1.08731C108.504 1.08776 108.504 1.08813 108.504 1.08904C108.504 1.09086 108.504 1.094 108.505 1.09846C108.505 1.10737 108.507 1.12154 108.508 1.14087C108.511 1.17953 108.516 1.23884 108.521 1.3181C108.531 1.4766 108.546 1.71483 108.559 2.02701C108.587 2.65144 108.613 3.57146 108.605 4.74115C108.589 7.08109 108.439 10.4169 107.899 14.382C106.82 22.3177 104.186 32.7396 97.9828 42.744C91.788 52.7348 82.0173 62.3373 66.6056 68.6424C51.181 74.9527 30.0502 77.9824 1.11396 74.7195L0.889857 76.7069C30.0626 79.9965 51.5486 76.9632 67.3629 70.4934C83.19 64.0184 93.2802 54.1235 99.6826 43.7979C106.077 33.4859 108.776 22.7723 109.881 14.6516C110.434 10.5886 110.589 7.16595 110.605 4.75453C110.613 3.54854 110.586 2.59464 110.557 1.93907C110.543 1.61126 110.528 1.35796 110.516 1.18495C110.511 1.09844 110.506 1.03199 110.502 0.986334C110.5 0.963505 110.499 0.945872 110.498 0.933526C110.497 0.927352 110.497 0.922499 110.497 0.918979C110.497 0.91722 110.496 0.915583 110.496 0.914701C110.496 0.913399 110.496 0.91243 109.5 1Z" fill="#FA7846"></path>
                </svg>
                <a href="#" class="default-btn js-fast_order--royal" data-gift="0">{{trans('portrait_royal.s_service_btn_price')}}</a>
            </div>
        </div>
    </div>
</section>
