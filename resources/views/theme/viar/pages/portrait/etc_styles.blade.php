                    ...
                    <div class="arts__image arts__image_vertical">
                        <picture>
                            ...
                            <img data-src="images/vertical.webp" src="{{ asset('images/vertical.webp') }}"
                                 class="arts__bg" @frontendAlt('theme/viar/pages/portrait/etc_styles.blade.php', 'images/vertical.webp', '', '')/>
                        </picture>
                        <picture class="arts__product-image">
                            @php
                                $portraitPageStyleImageSources = image_picture_sources(data_get($item, 'etc_style_image'), true);
                            @endphp
                            @if(!empty($portraitPageStyleImageSources['src_webp']))
                                <source srcset="{{ $portraitPageStyleImageSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($portraitPageStyleImageSources['src']) && !empty($portraitPageStyleImageSources['type']))
                                <source srcset="{{ $portraitPageStyleImageSources['src'] }}" type="{{ $portraitPageStyleImageSources['type'] }}">
                            @endif
                            <img class="" src="{{ $portraitPageStyleImageSources['src'] }}" @altAttrs($item, 'etc_style_image', data_get($item, 'etc_style_image'))/>
                        </picture>
                    </div>
