								...
								<div
									...
									<label>
										<span>{{ $item->getTranslatedAttribute('name', app()->getLocale()) }}
											<img src="{{ asset('images/icon/info.svg') }}"  @frontendAlt('theme/viar/pages/portrait/form/art_decor.blade.php', (asset('images/icon/info.svg')), '', '')>
										</span>
										...
										@if ($item->image)
											<div class="pic-pop">
												<picture>
													@php
														$portraitArtDecorImageSources = image_picture_sources(data_get($item, 'image'), true);
													@endphp
													@if(!empty($portraitArtDecorImageSources['src_webp']))
														<source srcset="{{ $portraitArtDecorImageSources['src_webp'] }}" type="image/webp">
													@endif
													@if(!empty($portraitArtDecorImageSources['src']) && !empty($portraitArtDecorImageSources['type']))
														<source srcset="{{ $portraitArtDecorImageSources['src'] }}" type="{{ $portraitArtDecorImageSources['type'] }}">
													@endif
													<img loading="lazy" src="{{ $portraitArtDecorImageSources['src'] }}" @altAttrs($item, 'image', data_get($item, 'image'))>
												</picture>
	...
	<div class="formalization-prompt">
		<div class="formalization-prompt--wrapper">
			<div class="formalization-prompt--inner">
				<img src="{{ asset('images/prompt6.png') }}"   @frontendAlt('theme/viar/pages/portrait/form/art_decor.blade.php', (asset('images/prompt6.png')), '', '')/>
				<p>
