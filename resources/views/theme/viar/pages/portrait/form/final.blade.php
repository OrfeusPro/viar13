        ...
        <div class="kviz-c-row kviz-c-group">
            ...
            <div class="set__item kviz-radio js_set_item js-checkbox @if($loop->last) kviz-radio_active @endif" data-stock="1">
                ...
                <div class="pic-pop">
                                    <picture>
                                        @php
                                            $portraitSetImageSources = image_picture_sources(data_get($set, 'image'), true);
                                        @endphp
                                        @if(!empty($portraitSetImageSources['src_webp']))
                                            <source srcset="{{ $portraitSetImageSources['src_webp'] }}" type="image/webp">
                                        @endif
                                        @if(!empty($portraitSetImageSources['src']) && !empty($portraitSetImageSources['type']))
                                            <source srcset="{{ $portraitSetImageSources['src'] }}" type="{{ $portraitSetImageSources['type'] }}">
                                        @endif
                                        <img loading="lazy" src="{{ $portraitSetImageSources['src'] }}" @altAttrs($set, 'image', data_get($set, 'image'))>
                                    </picture>
                                </div>
            </div>
            @endforeach
