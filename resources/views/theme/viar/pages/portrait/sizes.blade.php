<div class="sizes__screen section-p">
    <div class="sizes__screen--inner">
        <picture>
            ...
            <img src="{{ asset('images/sizes/size-bg.webp') }}"  @frontendAlt('theme/viar/pages/portrait/sizes.blade.php', (asset('images/sizes/size-bg.webp')), 'img', '')/>
        </picture>
        <picture>
            ...
            <img class="boy" src="{{ asset('images/sizes/10.webp') }}"  @frontendAlt('theme/viar/pages/portrait/sizes.blade.php', (asset('images/sizes/10.webp')), '', '')/>
        </picture>
        <picture>
            <source srcset="{{ asset('images/sizes/9.webp') }}" type="image/webp"/>
            <source srcset="{{ asset('images/sizes/9.png') }}" type="image/png"/>
            <img class="girl" src="{{ asset('images/sizes/9.webp') }}"  @frontendAlt('theme/viar/pages/portrait/sizes.blade.php', (asset('images/sizes/9.webp')), '', '')/>
        </picture>
        <div class="size-frame">
            ...
            <div class="sizes__block">
                @php
                    $sz_new_items = $item->getMedia('new_sizes');
                @endphp
                @if(!$sz_new_items->isEmpty())
                     @foreach($sz_new_items as $sz_item)
                        ...
                        @if($loop->iteration === 8) h @endif ">
                            ...
                            <picture>
                                @php
                                    $portraitPageSizeImageSources = image_picture_sources($sz_item->getUrl(), true);
                                @endphp
                                @if(!empty($portraitPageSizeImageSources['src_webp']))
                                    <source srcset="{{ $portraitPageSizeImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($portraitPageSizeImageSources['src']) && !empty($portraitPageSizeImageSources['type']))
                                    <source srcset="{{ $portraitPageSizeImageSources['src'] }}" type="{{ $portraitPageSizeImageSources['type'] }}">
                                @endif
                                <img src="{{ $portraitPageSizeImageSources['src'] }}" @altAttrs($item, 'media:new_sizes', $sz_item->getUrl(), null, $sz_item->getCustomProperty('image_alt_' . app()->getLocale()), $sz_item->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endforeach
                @endif
