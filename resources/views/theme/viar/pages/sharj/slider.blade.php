<div class="portraits portrait__screen new-screen new-hero hb_slider_sharj">
					...
					<div class="portraits-slide sl-slider pp-screen">
						<div class="m-screen-slide">
							<div class="pmain__screen">
								@php
									$sharjSlideImageSources = image_picture_sources(data_get($slide, 'image'), true);
								@endphp
								<picture>
									@if(!empty($sharjSlideImageSources['src_webp']))
										<source srcset="{{ $sharjSlideImageSources['src_webp'] }}" type="image/webp">
									@endif
									@if(!empty($sharjSlideImageSources['src']) && !empty($sharjSlideImageSources['type']))
										<source srcset="{{ $sharjSlideImageSources['src'] }}" type="{{ $sharjSlideImageSources['type'] }}">
									@endif
									<img width="1920" height="900" src="{{ $sharjSlideImageSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image', data_get($slide, 'image'), null, 'Viar Image')>
								</picture>

								<div class="pm-object sharj-pm-object">
									@php
										$sharjSlidePngSources = image_picture_sources(data_get($slide, 'png'), true);
									@endphp
									<picture>
										@if(!empty($sharjSlidePngSources['src_webp']))
											<source srcset="{{ $sharjSlidePngSources['src_webp'] }}" type="image/webp">
										@endif
										@if(!empty($sharjSlidePngSources['src']) && !empty($sharjSlidePngSources['type']))
											<source srcset="{{ $sharjSlidePngSources['src'] }}" type="{{ $sharjSlidePngSources['type'] }}">
										@endif
										<img width="578" height="589" src="{{ $sharjSlidePngSources['src'] }}"  @frontendAlt('theme/viar/pages/sharj/slider.blade.php', ($sharjSlidePngSources['src']), 'Viar Image', '')>
									</picture>
									{{-- <div class="pmo-block pmo-block1">
                                        <p>@lang("simpson.img_badge")</p>
                                        <img width="52" height="80" src="{{ asset(config('theme.current') . '/images/sharj/new/pngwing.webp') }}" alt="">
                                    </div> --}}
								</div>
									...
									<div class="pms-block pms-block1">
										<picture>
											<img width="150" height="195" src="{{ asset(config('theme.current') . '/images/sharjs.png') }}"
												 @frontendAlt('theme/viar/pages/sharj/slider.blade.php', (asset(config('theme.current') . '/images/sharjs.png')), 'Viar Image', '')>
										</picture>
									</div>
									<div class="mobile-img sharj-mobile-img">
										@php
											$sharjSlideMobileSources = image_picture_sources(data_get($slide, 'image_mob'), true);
										@endphp
										<picture>
											@if(!empty($sharjSlideMobileSources['src_webp']))
												<source srcset="{{ $sharjSlideMobileSources['src_webp'] }}" type="image/webp">
											@endif
											@if(!empty($sharjSlideMobileSources['src']) && !empty($sharjSlideMobileSources['type']))
												<source srcset="{{ $sharjSlideMobileSources['src'] }}" type="{{ $sharjSlideMobileSources['type'] }}">
											@endif
											<img width="370" height="500" src="{{ $sharjSlideMobileSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image_mob', data_get($slide, 'image_mob'), null, 'Viar Image')>
										</picture>
									</div>
									<div class="pmm-block">
										<p>@lang('sharj.translate44')</p>
										<img width="52" height="80" src="{{ asset(config('theme.current') . '/images/wing.svg') }}"
											 @frontendAlt('theme/viar/pages/sharj/slider.blade.php', (asset(config('theme.current') . '/images/wing.svg')), '', '')>
									</div>
								</div>
								...
								<div class="mp__inner">
									...
									<div class="section-frame">
										...
										<div class="portraits-btn mp-btn">
											<a href="#sharj" tabindex="-1">{{ $slide['btn'] }}</a>
										</div>
									</div>
								</div>
...
<div class="ellipse notmobile">
	<img  src="{{ asset(env('THEME') . 'images') }}/icon/ellipse-whete.svg" decoding="async"
		height="99" width="1374"  @frontendAlt('theme/viar/pages/sharj/slider.blade.php', (asset(env('THEME') . 'images')) . '/icon/ellipse-whete.svg', 'img', '')/>
</div>
