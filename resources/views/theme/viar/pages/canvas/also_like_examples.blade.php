            ...
            <div class="others-slider">
                @if($etc_styles_items)
                    @foreach($etc_styles_items as $item)
                        @php
                        $item_url = \App::getLocale() === 'ru' ? '' : '/'.\App::getLocale();
                        ...
                        switch ($item->getTranslatedAttribute('slug')) {
                            ...
                            default:
                                $item_url .= '/new/graphic-portrait/' . $item->getTranslatedAttribute('slug');
                                break;
                        }
                        @endphp
                        <div class="arts__item">
                            ...
                            <div class="arts__image arts__image_vertical">
                                ...
                                <picture class="arts__product-image hb_alt_none">
                                    @php
                                        $canvasStyleImageSources = image_picture_sources(data_get($item, 'etc_style_image'), true);
                                    @endphp
                                    @if(!empty($canvasStyleImageSources['src_webp']))
                                        <source srcset="{{ $canvasStyleImageSources['src_webp'] }}" type="image/webp">
                                    @endif
                                    @if(!empty($canvasStyleImageSources['src']) && !empty($canvasStyleImageSources['type']))
                                        <source srcset="{{ $canvasStyleImageSources['src'] }}" type="{{ $canvasStyleImageSources['type'] }}">
                                    @endif
                                    <img class="" src="{{ $canvasStyleImageSources['src'] }}" @altAttrs($item, 'etc_style_image', data_get($item, 'etc_style_image'))

                                    {{-- @if ($item->getCustomProperty('image_title_'.Config::get('app.locale')))
                                        title="{{ str_trans('image_title_'.Config::get('app.locale')) }}"
                                    @else

                                    @endif

                                    @if ($item->getCustomProperty('image_alt_'.Config::get('app.locale')))
                                        alt="{{ str_trans('image_alt_'.Config::get('app.locale')) }}"
                                    @else
                                    @endif --}}
                                    />
