<div class="portraits portrait__screen new-screen canvas-portrait hb_slider_canvas">
	<div class="portraits-wrap">
		<div class="portraits-slider">
			@if ($home_slides)
				@foreach ($home_slides as $slide)
					<div class="portraits-slide canvas-slide sl-slider @if (isset($slide['is_white']) && $slide['is_white']) mslidewhite @endif">
						<div class="m-screen-slide">
							<div class="pmain__screen">
								<picture>
									...
									<img loading="lazy" width="1920" height="900" src="{{ Voyager::image($slide['image']) }}" @altAttrs(['type' => \App\Models\CanvasSlider::class, 'id' => $slide['id'] ?? null], 'image', data_get($slide, 'image'), null, 'Viar Image')>
								</picture>
								<div class="pm-object">
									<picture>
										@if (isset($slide['avif']) && $slide['avif'])
											@php
												$new_file = json_decode($slide['avif'])[0];
												$new_file_path = Voyager::image($new_file->download_link);
											@endphp
											<source srcSet="{{ $new_file_path }}" type="image/avif" />
										@endif
									...
									@if (isset($slide['sub_title']) && $slide['sub_title'] && $slide['sub_title'] != '-')
										<div class="pmo-block pmo-block1 pmo-blockr @if (isset($slide['is_white']) && $slide['is_white']) slider_hb_white @endif">
											{!! $slide['sub_title'] !!}
										</div>
									@endif
								</div>
								<div class="mp-m-bg">
									<picture>
										@if (isset($slide['avif_mob']) && $slide['avif_mob'])
											@php
												$new_file = json_decode($slide['avif_mob'])[0];
												$new_file_path = Voyager::image($new_file->download_link);
											@endphp
											<source srcSet="{{ $new_file_path }}" type="image/avif" />
										@endif
										...
										<img loading="lazy" width="520" height="400" src="{{ Voyager::image($slide['image_mob']) }}"
											@altAttrs(['type' => \App\Models\CanvasSlider::class, 'id' => $slide['id'] ?? null], 'image_mob', data_get($slide, 'image_mob'), null, 'Viar Image')>
									</picture>
								</div>
								<div class="mp__inner">
...
{{-- <div class="marquee-container">
	<div class="marquee-inner">
		<span class="marquee-text">📢 Купите 2 постера на холсте и получите скидку 10% - Применить код скидки BUY2SAVE10</span>
		<span class="marquee-text">🎁 При покупке 5 постеров получите 1-й бесплатно - Применить код скидки BUY5GET1</span>
		<span class="marquee-text">✨ Получите 20% скидку на следующий заказ при покупке любого постера - Применить код скидки NEXT20</span>
		<span class="marquee-text">🎉 Купите 1 постер и получите эксклюзивный подарок - Применить код скидки GIFT1</span>
		
		<!-- Дублирование содержимого для бесшовного цикла -->
		<span class="marquee-text">📢 Купите 2 постера на холсте и получите скидку 10% - Применить код скидки BUY2SAVE10</span>
		<span class="marquee-text">🎁 При покупке 5 постеров получите 1-й бесплатно - Применить код скидки BUY5GET1</span>
		<span class="marquee-text">✨ Получите 20% скидку на следующий заказ при покупке любого постера - Применить код скидки NEXT20</span>
		<span class="marquee-text">🎉 Купите 1 постер и получите эксклюзивный подарок - Применить код скидки GIFT1</span>
