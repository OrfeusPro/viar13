...
<div class="accordion-content">
                    ...
                    <div class="kviz-radio js-checkbox @if($loop->last) kviz-radio_active @endif" data-stock="{{ $loop->iteration }}">
                            ...
                            <label>
                                                    <span>{{ $item->getTranslatedAttribute('name', app()->getLocale()) }}
                                                      <img src="{{ asset('images/icon/info.svg') }}"  @frontendAlt('theme/viar/pages/canvas/form/step4_show.blade.php', (asset('images/icon/info.svg')), '', '')></span>
    ...
    {{--                                согласуйте с нами.--}}
                                ...
                                <div class="pic-pop">
                                    <picture>
                                        @php
                                            $canvasStepFourImageSources = image_picture_sources(data_get($item, 'image'), true);
                                        @endphp
                                        @if(!empty($canvasStepFourImageSources['src_webp']))
                                            <source srcset="{{ $canvasStepFourImageSources['src_webp'] }}" type="image/webp">
                                        @endif
                                        @if(!empty($canvasStepFourImageSources['src']) && !empty($canvasStepFourImageSources['type']))
                                            <source srcset="{{ $canvasStepFourImageSources['src'] }}" type="{{ $canvasStepFourImageSources['type'] }}">
                                        @endif
                                        <img loading="lazy" src="{{ $canvasStepFourImageSources['src'] }}" @altAttrs($item, 'image', data_get($item, 'image'))>
                                    </picture>
