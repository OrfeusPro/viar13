...
<h2 class="page-title ">
    @if (Route::currentRouteName() == 'hb.gallery.painters')
        @lang('gallery.paintes_title')
    @elseif (Route::currentRouteName() == 'hb.gallery.modern_painters')
        @lang('gallery.painters_modern_title')
    @else
        @lang('gallery.painters_default_title')
    @endif
</h2><br>
								...
								<div class="cp-item">
									<div class="cp-item-inner">
										<div class="cp-item-inner-b">
											<a href="{{ route('hb.gallery.category', ['type' => $type, 'category' => $item->url]) }}"
												class="mm-btn">@lang('gallery.see')</a>
										</div>
										<picture>
											@if ($item->image)
                                                @php
                                                    $galleryPainterImageSources = image_picture_sources($item->image, true);
                                                @endphp
                                                @if(!empty($galleryPainterImageSources['src_webp']))
                                                    <source srcset="{{ $galleryPainterImageSources['src_webp'] }}" type="image/webp">
                                                @endif
                                                @if(!empty($galleryPainterImageSources['src']) && !empty($galleryPainterImageSources['type']))
                                                    <source srcset="{{ $galleryPainterImageSources['src'] }}" type="{{ $galleryPainterImageSources['type'] }}">
                                                @endif
												<img width="433" height="583" src="{{ $galleryPainterImageSources['src'] }}" @altAttrs($item, 'image', data_get($item, 'image')) loading="lazy">
											@else
...
<h2 class="page-title ">
    @if (Route::currentRouteName() == 'hb.gallery.painters')
        @lang('gallery.paintes_search')
    @elseif (Route::currentRouteName() == 'hb.gallery.modern_painters')
        @lang('gallery.painters_modern_search')
    @else
        @lang('gallery.paintes_default_search')
    @endif
</h2><br>
