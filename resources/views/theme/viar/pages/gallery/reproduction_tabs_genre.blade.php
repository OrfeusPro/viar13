<div class="rp-block rp-century rp-type hidden-block">
	<div class="rp-search">
		<form class="mc-search" action="{{ route("hb.gallery.list_genre") }}">
			<div class="mc-input">
	...
	<div class="rp-century-row">
		<div class="select-century">
			<div class="p-text">
				@lang('gallery.select_genre'):
			</div>
			<ul>
				@foreach ($gallery_genre as $genre)
					<li><a href="{{ route('hb.gallery.genre', ['alias' => $genre->alias]) }}">{{ translated_value($genre, 'name', $genre->name) }}</a></li>
				@endforeach
			</ul>
		</div>
				...
				@foreach ($colors as $color)
					@if(isset($category) && $category)
						<li style="padding-right: 0px;"><a
								href="{{ route('hb.gallery.category', [$page->url, $category->url, 'color' => $color->id, 'size' => \Request::get('size') ?? '', 'order' => \Request::get('order') ?? '']) }}"
								name="{{ $color->id }}">
								<div style="background: {{ $color->color }}; height: 29px; border: 1px solid #f3f3f3; width: 100%;"></div>
							</a></li>
					@else
						<li style="padding-right: 0px;"><a
								href="{{ route('hb.gallery.module', [$page->url, 'color' => $color->id, 'size' => \Request::get('size') ?? '', 'order' => \Request::get('order') ?? '']) }}"
								name="{{ $color->id }}">
								<div style="background: {{ $color->color }}; height: 29px; border: 1px solid #f3f3f3:width: 100%;"></div>
							</a></li>
					@endif
