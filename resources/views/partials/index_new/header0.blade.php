								...
								@if ($item['images'])
									@foreach (json_decode($item['images']) as $img)
										<a href="#" class="vz-art menu-item">
                                            @php
                                                $homeMenuImageSources = image_picture_sources($img, true);
                                            @endphp
											<picture>
                                                @if(!empty($homeMenuImageSources['src_webp']))
                                                    <source srcset="{{ $homeMenuImageSources['src_webp'] }}" type="image/webp">
                                                @endif
                                                ...
                                                @endif
												<img class="lozad" src="{{ $homeMenuImageSources['src'] }}" loading="lazy"  @frontendAlt('partials/index_new/header0.blade.php', ($homeMenuImageSources['src']), '', '')>
											</picture>
										</a>
									@endforeach
								@endif
...
<div class="vz-art burge-menu">
    <div class="vz-art header-bar header-bar_burger">
        <a href="{{ route('home') }}" class="vz-art logo" aria-label="logo link">
            <img loading="lazy" src="{{ asset('images/logo.svg') }}"  @frontendAlt('partials/index_new/header0.blade.php', (asset('images/logo.svg')), __('settings.site_name'), '')>
        </a>
    ...
    <div class="vz-art burge-menu__list">
        ...
        <div class="vz-art burge-menu__item">
            <div  class="h3_old">{{ trans('header_footer_new.header_col3_name') }} <i class="fa-arrow-down"></i>
            </div>
            @if (isset($menu_items3) && $menu_items3)
                <ul>
                    @foreach ($menu_items3 as $item)
    ...
    <div class="vz-art language header-item_pc _spollers _one _esc new_lang">
                <p class="_spoller "><img class="lozad" loading="lazy" src="{{ asset('images/flag/' . app()->getLocale() . '.svg') }}" @frontendAlt('partials/index_new/header0.blade.php', (asset('images/flag/' . app()->getLocale() . '.svg')), '', '')> {{ ucfirst(app()->getLocale()) }} <i class="fa-arrow-down"></i>
                </p>
                ...
                <ul class="ver1_langs">
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        ...
                        <li>
                            <a href="{{ $cur_url_mod }}">
                                <img class="lozad" src="{{ asset('images/flag/' . $localeCode . '.svg') }}" loading="lazy" @frontendAlt('partials/index_new/header0.blade.php', (asset('images/flag/' . $localeCode . '.svg')), '', '')> {{ $localeCode }}
                            </a>
                        </li>
                    @endforeach
                </ul>
