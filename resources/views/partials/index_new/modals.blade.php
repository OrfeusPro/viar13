...
<div class="vz-art popup-frame target-frame">
	...
	{{-- portrait by photo -header --}}
	@include(env('THEME_RESOURCES') . 'modals.quiz_form')
	@include(env('THEME_RESOURCES') . 'modals.target-box')
	@include(env('THEME_RESOURCES') . 'modals.popup_add_to_cart')
	...
	@if (isset($is_portrait_page))
		<form data-portrait-by-photo-header="1" action="{{ route('send_photo_portrait_form') }}" method="POST"
			...
			<div class="vz-art page-title popup-photo-title">{{ trans('homepage_new.order_photo_portrait_title') }}</div>
