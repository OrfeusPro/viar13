                ...
                <div class="m-screen-slide">
                    <div class="pmain__screen">
                        <picture>
                            ...
                            @php
                                $simpsonsSlideImageSources = image_picture_sources(data_get($slide, 'image'), true);
                            @endphp
                            @if(!empty($simpsonsSlideImageSources['src_webp']))
                                <source srcset="{{ $simpsonsSlideImageSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($simpsonsSlideImageSources['src']) && !empty($simpsonsSlideImageSources['type']))
                                <source srcset="{{ $simpsonsSlideImageSources['src'] }}" type="{{ $simpsonsSlideImageSources['type'] }}">
                            @endif
                            <img width="1920" height="900" src="{{ $simpsonsSlideImageSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image', data_get($slide, 'image'), null, 'Viar Image')>
                        </picture>
                        <div class="pm-object">
                            <picture>
                                @php
                                    $simpsonsSlidePngSources = image_picture_sources(data_get($slide, 'png'), true);
                                @endphp
                                @if(!empty($simpsonsSlidePngSources['src_webp']))
                                    <source srcset="{{ $simpsonsSlidePngSources['src_webp'] }}" type="image/webp">
                                @endif
                            ...
                            <div class="pms-block">
                                <picture>
                                    @php
                                        $simpsonsSlidePhotoSources = image_picture_sources(data_get($slide, 'fotopng'), true);
                                    @endphp
                                    @if(!empty($simpsonsSlidePhotoSources['src_webp']))
                                        <source srcset="{{ $simpsonsSlidePhotoSources['src_webp'] }}" type="image/webp">
                                    @endif
                        ...
                        <div class="mp-m-bg">
                            ...
                            <div class="pms-block pms-block1">
                                <picture>
                                    @php
                                        $simpsonsSlideMobilePhotoSources = image_picture_sources(data_get($slide, 'fotopng'), true);
                                    @endphp
                                    @if(!empty($simpsonsSlideMobilePhotoSources['src_webp']))
                                        <source srcset="{{ $simpsonsSlideMobilePhotoSources['src_webp'] }}" type="image/webp">
                                    @endif
                            ...
                            <picture>
                                ...
                                @php
                                    $simpsonsSlideMobileSources = image_picture_sources(data_get($slide, 'image_mob'), true);
                                @endphp
                                @if(!empty($simpsonsSlideMobileSources['src_webp']))
                                    <source media="(max-width: 520px)" srcset="{{ $simpsonsSlideMobileSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($simpsonsSlideMobileSources['src']) && !empty($simpsonsSlideMobileSources['type']))
                                    <source media="(max-width: 520px)" srcset="{{ $simpsonsSlideMobileSources['src'] }}" type="{{ $simpsonsSlideMobileSources['type'] }}">
                                @endif
                                <img width="520" height="400" src="{{ $simpsonsSlideMobileSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image_mob', data_get($slide, 'image_mob'), null, 'Viar Image')>
                            </picture>
                            @if(isset($slide['sub_title']) && $slide['sub_title'] && $slide['sub_title']!='-')
                            <div class="pmm-block">
                                <p>{!! $slide['sub_title'] !!}</p>
