								...
								<div
									...
									<label>
										<span>{{ $item->getTranslatedAttribute('name', app()->getLocale()) }}
											<img src="{{ asset('images/icon/info.svg') }}"  @frontendAlt('partials/simpsons/form/art_decor.blade.php', (asset('images/icon/info.svg')), '', '')>
										</span>
										...
										@if ($item->image)
											<div class="pic-pop">
												<picture>
													@php
														$simpsonsArtDecorImageSources = image_picture_sources(data_get($item, 'image'), true);
													@endphp
													@if(!empty($simpsonsArtDecorImageSources['src_webp']))
														<source srcset="{{ $simpsonsArtDecorImageSources['src_webp'] }}" type="image/webp">
													@endif
													@if(!empty($simpsonsArtDecorImageSources['src']) && !empty($simpsonsArtDecorImageSources['type']))
														<source srcset="{{ $simpsonsArtDecorImageSources['src'] }}" type="{{ $simpsonsArtDecorImageSources['type'] }}">
													@endif
													<img loading="lazy" src="{{ $simpsonsArtDecorImageSources['src'] }}" @altAttrs($item, 'image', data_get($item, 'image'))>
												</picture>
	...
	<div class="formalization-prompt">
		<div class="formalization-prompt--wrapper">
			<div class="formalization-prompt--inner">
				<img src="{{ asset('images/prompt6.png') }}"   @frontendAlt('partials/simpsons/form/art_decor.blade.php', (asset('images/prompt6.png')), '', '')/>
				<p>
