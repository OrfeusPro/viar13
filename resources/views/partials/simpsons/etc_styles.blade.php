                    ...
                    <div class="arts__image arts__image_vertical">
                        <picture>
                            ...
                            <img data-src="images/vertical.webp" src="{{ asset('images/vertical.webp') }}"
                                 class="arts__bg" @frontendAlt('partials/simpsons/etc_styles.blade.php', 'images/vertical.webp', '', '')/>
                        </picture>
                        <picture class="arts__product-image">
                            @php
                                $simpsonsStyleImageSources = image_picture_sources(data_get($item, 'etc_style_image'), true);
                            @endphp
                            @if(!empty($simpsonsStyleImageSources['src_webp']))
                                <source srcset="{{ $simpsonsStyleImageSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($simpsonsStyleImageSources['src']) && !empty($simpsonsStyleImageSources['type']))
                                <source srcset="{{ $simpsonsStyleImageSources['src'] }}" type="{{ $simpsonsStyleImageSources['type'] }}">
                            @endif
                            <img class="" src="{{ $simpsonsStyleImageSources['src'] }}" @altAttrs($item, 'etc_style_image', data_get($item, 'etc_style_image'))/>
                        </picture>
                    </div>
