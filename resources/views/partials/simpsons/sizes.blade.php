                ...
                @if(!$sz_new_items->isEmpty())
                     @foreach($sz_new_items as $sz_item)
                        ...
                        @if($loop->iteration === 8) h @endif ">
                            ...
                            <picture>
                                @php
                                    $simpsonsSizeImageSources = image_picture_sources($sz_item->getUrl(), true);
                                @endphp
                                @if(!empty($simpsonsSizeImageSources['src_webp']))
                                    <source srcset="{{ $simpsonsSizeImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($simpsonsSizeImageSources['src']) && !empty($simpsonsSizeImageSources['type']))
                                    <source srcset="{{ $simpsonsSizeImageSources['src'] }}" type="{{ $simpsonsSizeImageSources['type'] }}">
                                @endif
                                <img src="{{ $simpsonsSizeImageSources['src'] }}" @altAttrs($item, 'media:new_sizes', $sz_item->getUrl(), null, $sz_item->getCustomProperty('image_alt_' . app()->getLocale()), $sz_item->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endforeach
                @endif
