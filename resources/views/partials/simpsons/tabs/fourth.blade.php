                    ...
                    <div class="examples-slide">
                        <a href="{{ $s_item->getUrl() }}" class="examples-slide__photo"
                           data-fancybox="examples" aria-label="">
                            <picture>
                                @php
                                    $simpsonsTabExampleImageSources = image_picture_sources($s_item->getUrl(), true);
                                @endphp
                                @if(!empty($simpsonsTabExampleImageSources['src_webp']))
                                    <source srcset="{{ $simpsonsTabExampleImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($simpsonsTabExampleImageSources['src']) && !empty($simpsonsTabExampleImageSources['type']))
                                    <source srcset="{{ $simpsonsTabExampleImageSources['src'] }}" type="{{ $simpsonsTabExampleImageSources['type'] }}">
                                @endif
                                <img src="{{ $simpsonsTabExampleImageSources['src'] }}" @altAttrs($item, 'media:our_works_new', $s_item->getUrl(), null, $s_item->getCustomProperty('image_alt_' . app()->getLocale()), $s_item->getCustomProperty('image_title_' . app()->getLocale())) loading="lazy"/>
                            </picture>
                        </a>
