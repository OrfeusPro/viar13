    ...
    <div class="about-size--inner">
        ...
        @endphp
         @if(!$sp_items->isEmpty())
            ...
            <picture>
                @php
                    $simpsonsPriceSizeImageSources = image_picture_sources($s_item->getUrl(), true);
                @endphp
                @if(!empty($simpsonsPriceSizeImageSources['src_webp']))
                    <source srcset="{{ $simpsonsPriceSizeImageSources['src_webp'] }}" type="image/webp">
                @endif
                @if(!empty($simpsonsPriceSizeImageSources['src']) && !empty($simpsonsPriceSizeImageSources['type']))
                    <source srcset="{{ $simpsonsPriceSizeImageSources['src'] }}" type="{{ $simpsonsPriceSizeImageSources['type'] }}">
                @endif
                <img src="{{ $simpsonsPriceSizeImageSources['src'] }}" @altAttrs($item, 'media:new_prices_sizes', $s_item->getUrl(), null, $s_item->getCustomProperty('image_alt_' . app()->getLocale()), $s_item->getCustomProperty('image_title_' . app()->getLocale()))/>
            </picture>
            <p>
