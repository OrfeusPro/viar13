<div class="portraits portrait__screen new-screen">
            ...
            <div class="portraits-slide sl-slider pp-screen">
                <div class="m-screen-slide">
                    <div class="pmain__screen">
                        <picture>
                            {{-- <source srcset="images/bg/canvas-bg.webp" type="image/webp"> --}}
                            @php
                                $portraitNewSlideImageSources = image_picture_sources(data_get($slide, 'image'), true);
                            @endphp
                            @if(!empty($portraitNewSlideImageSources['src_webp']))
                                <source srcset="{{ $portraitNewSlideImageSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($portraitNewSlideImageSources['src']) && !empty($portraitNewSlideImageSources['type']))
                                <source srcset="{{ $portraitNewSlideImageSources['src'] }}" type="{{ $portraitNewSlideImageSources['type'] }}">
                            @endif
                            <img width="1920" height="900" src="{{ $portraitNewSlideImageSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image', data_get($slide, 'image'), null, 'Viar Image')>
                        </picture>
                        <div class="pm-object">
                            <picture>
                                @php
                                    $portraitNewSlidePngSources = image_picture_sources(data_get($slide, 'png'), true);
                                @endphp
                                @if(!empty($portraitNewSlidePngSources['src_webp']))
                                    <source srcset="{{ $portraitNewSlidePngSources['src_webp'] }}" type="image/webp">
                                @endif
                            ...
                            <div class="pms-block">
                                <picture>
                                    @php
                                        $portraitNewSlidePhotoSources = image_picture_sources(data_get($slide, 'fotopng'), true);
                                    @endphp
                                    @if(!empty($portraitNewSlidePhotoSources['src_webp']))
                                        <source srcset="{{ $portraitNewSlidePhotoSources['src_webp'] }}" type="image/webp">
                                    @endif
                        ...
                        <div class="mp-m-bg">
                            ...
                            <div class="pms-block pms-block1">
                                <picture>
                                    @php
                                        $portraitNewSlideMobilePhotoSources = image_picture_sources(data_get($slide, 'fotopng'), true);
                                    @endphp
                                    @if(!empty($portraitNewSlideMobilePhotoSources['src_webp']))
                                        <source srcset="{{ $portraitNewSlideMobilePhotoSources['src_webp'] }}" type="image/webp">
                                    @endif
                            ...
                            @endif
                            <picture>
                                {{-- <source media="(max-width: 520px)" srcset="{{ asset(env('THEME').'images/bg/portrait-m.webp')}}" type="image/webp"> --}}
                                @php
                                    $portraitNewSlideMobileSources = image_picture_sources(data_get($slide, 'image_mob'), true);
                                @endphp
                                @if(!empty($portraitNewSlideMobileSources['src_webp']))
                                    <source media="(max-width: 520px)" srcset="{{ $portraitNewSlideMobileSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($portraitNewSlideMobileSources['src']) && !empty($portraitNewSlideMobileSources['type']))
                                    <source media="(max-width: 520px)" srcset="{{ $portraitNewSlideMobileSources['src'] }}" type="{{ $portraitNewSlideMobileSources['type'] }}">
                                @endif
                                <img width="520" height="400" src="{{ $portraitNewSlideMobileSources['src'] }}" @altAttrs(['type' => \App\Models\PortraitSlider::class, 'id' => $slide['id'] ?? null], 'image_mob', data_get($slide, 'image_mob'), null, 'Viar Image')>
                            </picture>
                            @if(isset($slide['sub_title']) && $slide['sub_title'] && $slide['sub_title']!='-')
                            <div class="pmm-block">
                                <p>{!! $slide['sub_title'] !!}</p>
