<section class="examples c-examples section-p" id="examples">
    <div class="section-frame">
        <h2 class="page-title page-title_white h2_old">{{ trans('portrait.examples_title') }}</h2>
            ...
            <div class="before-after__block">
                ...
                <div class="ba-slider">
                    @if(!$ba_items->isEmpty())
                        <picture>
                            @php
                                $portraitNewExampleBeforeSources = image_picture_sources($ba_items[0]->getUrl(), true);
                            @endphp
                            @if(!empty($portraitNewExampleBeforeSources['src_webp']))
                                <source srcset="{{ $portraitNewExampleBeforeSources['src_webp'] }}" type="image/webp">
                            @endif
                            @if(!empty($portraitNewExampleBeforeSources['src']) && !empty($portraitNewExampleBeforeSources['type']))
                                <source srcset="{{ $portraitNewExampleBeforeSources['src'] }}" type="{{ $portraitNewExampleBeforeSources['type'] }}">
                            @endif
                            <img src="{{ $portraitNewExampleBeforeSources['src'] }}" @altAttrs($item, 'media:new_before_items', $ba_items[0]->getUrl(), null, $ba_items[0]->getCustomProperty('image_alt_' . app()->getLocale()), $ba_items[0]->getCustomProperty('image_title_' . app()->getLocale()))/>
                        </picture>
                    @endif
                    @if(!$ba_items_after->isEmpty())
                        <div class="resize">
                            <picture>
                                @php
                                    $portraitNewExampleAfterSources = image_picture_sources($ba_items_after[0]->getUrl(), true);
                                @endphp
                                @if(!empty($portraitNewExampleAfterSources['src_webp']))
                                    <source srcset="{{ $portraitNewExampleAfterSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($portraitNewExampleAfterSources['src']) && !empty($portraitNewExampleAfterSources['type']))
                                    <source srcset="{{ $portraitNewExampleAfterSources['src'] }}" type="{{ $portraitNewExampleAfterSources['type'] }}">
                                @endif
                                <img srcset="{{ $portraitNewExampleAfterSources['src'] }}" @altAttrs($item, 'media:new_after_items', $ba_items_after[0]->getUrl(), null, $ba_items_after[0]->getCustomProperty('image_alt_' . app()->getLocale()), $ba_items_after[0]->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endif
                                ...
                                <a class="examples-slide__photo">
                                        <picture>
											@if(isset($ba_items2[$loop->index]) && $ba_items2[$loop->index]->getUrl())
                                                @php
                                                    $portraitNewExampleWorkSources = image_picture_sources($ba_item->getUrl(), true);
                                                @endphp
                                                @if(!empty($portraitNewExampleWorkSources['src_webp']))
                                                    <source srcset="{{ $portraitNewExampleWorkSources['src_webp'] }}" type="image/webp">
                                                @endif
                                                @if(!empty($portraitNewExampleWorkSources['src']) && !empty($portraitNewExampleWorkSources['type']))
                                                    <source srcset="{{ $portraitNewExampleWorkSources['src'] }}" type="{{ $portraitNewExampleWorkSources['type'] }}">
                                                @endif
                                            <img src="{{ $portraitNewExampleWorkSources['src'] }}" data-fancybox="group" data-src="{{ $ba_items2[$loop->index]->getUrl() }}" @altAttrs($item, 'media:works_examples_new', $ba_item->getUrl(), null, $ba_item->getCustomProperty('image_alt_' . app()->getLocale()), $ba_item->getCustomProperty('image_title_' . app()->getLocale())) loading="lazy"/>
											@endif
                                        </picture>
                                </a>
