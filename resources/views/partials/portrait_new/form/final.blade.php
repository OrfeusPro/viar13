            ...
            <div class="set__item kviz-radio js_set_item js-checkbox @if($loop->last) kviz-radio_active @endif" data-stock="1">
                ...
                <label>
                    <span>{{ $set->getTranslatedAttribute('name', app()->getLocale()) }}
                        <img src="{{ asset('images/icon/info.svg') }}"   @frontendAlt('partials/portrait_new/form/final.blade.php', (asset('images/icon/info.svg')), '', '')/></span>
                ...
                <div class="pic-pop">
                                    <picture>
                                        @php
                                            $portraitNewSetImageSources = image_picture_sources(data_get($set, 'image'), true);
                                        @endphp
                                        @if(!empty($portraitNewSetImageSources['src_webp']))
                                            <source srcset="{{ $portraitNewSetImageSources['src_webp'] }}" type="image/webp">
                                        @endif
                                        @if(!empty($portraitNewSetImageSources['src']) && !empty($portraitNewSetImageSources['type']))
                                            <source srcset="{{ $portraitNewSetImageSources['src'] }}" type="{{ $portraitNewSetImageSources['type'] }}">
                                        @endif
                                        <img loading="lazy" src="{{ $portraitNewSetImageSources['src'] }}" @altAttrs($set, 'image', data_get($set, 'image'))> 
                                    </picture>
                                </div>
            </div>
            @endforeach
