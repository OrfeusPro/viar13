                                    ...
                                    <label class="kviz-radio js__frame_item js-checkbox" data-stock="1" data-price="{{ $frame->price }}">
                                        ...
                                        <div>
                                                <span>{{ $frame->getTranslatedAttribute('name', app()->getLocale()) }} + <b> {{ $frame->price }} €</b>
                                                    @php
                                                        $portraitNewFrameImageSources = image_picture_sources(data_get($frame, 'img_bg'), true);
                                                    @endphp
                                                    <picture>
                                                        @if(!empty($portraitNewFrameImageSources['src_webp']))
                                                            <source srcset="{{ $portraitNewFrameImageSources['src_webp'] }}" type="image/webp">
                                                        @endif
                                                        @if(!empty($portraitNewFrameImageSources['src']) && !empty($portraitNewFrameImageSources['type']))
                                                            <source srcset="{{ $portraitNewFrameImageSources['src'] }}" type="{{ $portraitNewFrameImageSources['type'] }}">
                                                        @endif
                                                        <img style="width:70px;height:18px;" src="{{ $portraitNewFrameImageSources['src'] }}" @altAttrs($frame, 'img_bg', data_get($frame, 'img_bg'))/>
                                                  </picture>
                                                </span>
                                            <input class="js_frame" type="radio" name="frame" data-price="{{ $frame->price }}" value="{{ $frame->id }}"/>
                                        </div>
    ...
    <div class="formalization-prompt">
        <div class="formalization-prompt--wrapper">
            <div class="formalization-prompt--inner">
                <img src="{{ asset('images/prompt7.png') }}"  @frontendAlt('partials/portrait_new/form/frames.blade.php', (asset('images/prompt7.png')), '', '')/>
                <p>
