					...
					<div class="kviz-group__item" style="width: fit-content;">
						@if (isset($art_items))
							@foreach ($art_items as $item)
								<div class="js_decor__item kv__size__item kviz-radio js-checkbox @if ($loop->last) kviz-radio_active @endif"
									data-stock="1">
									<div class="check"></div>
									<label>
										...
										@if ($item->image)
											<div class="pic-pop">
												<picture>
													@php
														$portraitNewCanvasOptionImageSources = image_picture_sources(data_get($item, 'image'), true);
													@endphp
													@if(!empty($portraitNewCanvasOptionImageSources['src_webp']))
														<source srcset="{{ $portraitNewCanvasOptionImageSources['src_webp'] }}" type="image/webp">
													@endif
													@if(!empty($portraitNewCanvasOptionImageSources['src']) && !empty($portraitNewCanvasOptionImageSources['type']))
														<source srcset="{{ $portraitNewCanvasOptionImageSources['src'] }}" type="{{ $portraitNewCanvasOptionImageSources['type'] }}">
													@endif
													<img loading="lazy" src="{{ $portraitNewCanvasOptionImageSources['src'] }}" @altAttrs($item, 'image', data_get($item, 'image'))>
												</picture>
