<div class="sizes__screen section-p">
    <div class="sizes__screen--inner">
        <picture>
            ...
            <img src="{{ asset('images/sizes/size-bg.webp') }}"  @frontendAlt('partials/portrait_new/sizes.blade.php', (asset('images/sizes/size-bg.webp')), 'img', '')/>
        </picture>
        <picture>
            ...
            <img class="boy" src="{{ asset('images/sizes/10.webp') }}"  @frontendAlt('partials/portrait_new/sizes.blade.php', (asset('images/sizes/10.webp')), '', '')/>
        </picture>
        <picture>
            <source srcset="{{ asset('images/sizes/9.webp') }}" type="image/webp"/>
            <source srcset="{{ asset('images/sizes/9.png') }}" type="image/png"/>
            <img class="girl" src="{{ asset('images/sizes/9.webp') }}"  @frontendAlt('partials/portrait_new/sizes.blade.php', (asset('images/sizes/9.webp')), '', '')/>
        </picture>
                ...
                @if(!$sz_new_items->isEmpty())
                     @foreach($sz_new_items as $sz_item)
                        ...
                        @if($loop->iteration === 8) h @endif ">
                            ...
                            <picture>
                                @php
                                    $portraitNewSizeImageSources = image_picture_sources($sz_item->getUrl(), true);
                                @endphp
                                @if(!empty($portraitNewSizeImageSources['src_webp']))
                                    <source srcset="{{ $portraitNewSizeImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($portraitNewSizeImageSources['src']) && !empty($portraitNewSizeImageSources['type']))
                                    <source srcset="{{ $portraitNewSizeImageSources['src'] }}" type="{{ $portraitNewSizeImageSources['type'] }}">
                                @endif
                                <img src="{{ $portraitNewSizeImageSources['src'] }}" @altAttrs($item, 'media:new_sizes', $sz_item->getUrl(), null, $sz_item->getCustomProperty('image_alt_' . app()->getLocale()), $sz_item->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endforeach
                @endif
