                        ...
                        <div class="arts__item">
                            ...
                            <div class="arts__image arts__image_vertical">
                                <picture>
                                    ...
                                    <img data-src="images/vertical.webp" src="{{ asset('images/vertical.webp') }}"
                                         class="arts__bg" @frontendAlt('partials/canvas_new/also_like_examples.blade.php', 'images/vertical.webp', '', '')/>
                                </picture>
                                <picture class="arts__product-image">
                                    @php
                                        $alsoLikeStyleImageSources = image_picture_sources(data_get($item, 'etc_style_image'), true);
                                    @endphp
                                    @if(!empty($alsoLikeStyleImageSources['src_webp']))
                                        <source srcset="{{ $alsoLikeStyleImageSources['src_webp'] }}" type="image/webp">
                                    @endif
                                    @if(!empty($alsoLikeStyleImageSources['src']) && !empty($alsoLikeStyleImageSources['type']))
                                        <source srcset="{{ $alsoLikeStyleImageSources['src'] }}" type="{{ $alsoLikeStyleImageSources['type'] }}">
                                    @endif
                                    <img class="" src="{{ $alsoLikeStyleImageSources['src'] }}" @altAttrs($item, 'etc_style_image', data_get($item, 'etc_style_image'))/>
                                </picture>
                            </div>
