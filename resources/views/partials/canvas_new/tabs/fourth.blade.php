                    ...
                    <div class="examples-slide">
                        <a href="{{ $s_item->getUrl() }}" class="examples-slide__photo"
                           data-fancybox="examples" aria-label="">
                            <picture>
                                @php
                                    $canvasNewTabExampleImageSources = image_picture_sources($s_item->getUrl(), true);
                                @endphp
                                @if(!empty($canvasNewTabExampleImageSources['src_webp']))
                                    <source srcset="{{ $canvasNewTabExampleImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($canvasNewTabExampleImageSources['src']) && !empty($canvasNewTabExampleImageSources['type']))
                                    <source srcset="{{ $canvasNewTabExampleImageSources['src'] }}" type="{{ $canvasNewTabExampleImageSources['type'] }}">
                                @endif
                                <img data-lazy="{{ $canvasNewTabExampleImageSources['src'] }}" @altAttrs($item, 'media:tab_examples', $s_item->getUrl(), null, $s_item->getCustomProperty('image_alt_' . app()->getLocale()), $s_item->getCustomProperty('image_title_' . app()->getLocale())) loading="lazy"/>
                            </picture>
                        </a>
