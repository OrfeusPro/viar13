                ...
                @if(!$sz_new_items->isEmpty())
                    @foreach($sz_new_items as $sz_item)
                        ...
                        @if($loop->iteration === 8) h @endif ">
                            ...
                            <picture>
                                @php
                                    $canvasNewSizeImageSources = image_picture_sources($sz_item->getUrl(), true);
                                @endphp
                                @if(!empty($canvasNewSizeImageSources['src_webp']))
                                    <source srcset="{{ $canvasNewSizeImageSources['src_webp'] }}" type="image/webp">
                                @endif
                                @if(!empty($canvasNewSizeImageSources['src']) && !empty($canvasNewSizeImageSources['type']))
                                    <source srcset="{{ $canvasNewSizeImageSources['src'] }}" type="{{ $canvasNewSizeImageSources['type'] }}">
                                @endif
                                <img src="{{ $canvasNewSizeImageSources['src'] }}" @altAttrs($item, 'media:sizes_bot_items', $sz_item->getUrl(), null, $sz_item->getCustomProperty('image_alt_' . app()->getLocale()), $sz_item->getCustomProperty('image_title_' . app()->getLocale()))/>
                            </picture>
                        </div>
                    @endforeach
                @endif
