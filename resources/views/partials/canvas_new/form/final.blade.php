                            ...
                            <div class="kviz-radio js-checkbox @if($loop->last) kviz-radio_active @endif" data-stock="1">
                              ...
                              >{{ $set->getTranslatedAttribute('name', app()->getLocale()) }}<img
                                ...
                                <div class="pic-pop">
                                    <picture>
                                        @php
                                            $canvasNewSetImageSources = image_picture_sources(data_get($set, 'image'), true);
                                        @endphp
                                        @if(!empty($canvasNewSetImageSources['src_webp']))
                                            <source srcset="{{ $canvasNewSetImageSources['src_webp'] }}" type="image/webp">
                                        @endif
                                        @if(!empty($canvasNewSetImageSources['src']) && !empty($canvasNewSetImageSources['type']))
                                            <source srcset="{{ $canvasNewSetImageSources['src'] }}" type="{{ $canvasNewSetImageSources['type'] }}">
                                        @endif
                                        <img loading="lazy" src="{{ $canvasNewSetImageSources['src'] }}" @altAttrs($set, 'image', data_get($set, 'image'))> 
                                    </picture>
                                </div>
                            </div>
                        @endforeach
