...
<div class="{{ $statusClass }} @if($key==count($orders)-1) last @endif" style="margin-top:20px;">
    <div class="{{ $statusClass }}-content">
        ...
        <ul class="data">
            ...
            <li style="vertical-align: top;"><strong>@lang('account.index17') </strong>@lang('account.index18')</li>

            @if(Auth::user()->role->name != 'painter')
            <li style="vertical-align: top;">
                @if($order->has_pdf == 1)
                <a target="_blank" href="/storage/pdf/{{ $order->id }}.pdf?ver={{ preg_replace('/\D/', '', $order->updated_at) }}">@lang('account.index19')</a>
                @else
        ...
        <div>
			...
			@foreach($order->items as $product)
				...
				<div class="{{ $statusClass }}-items clearfix">
					...
					<div class="acting-item clearfix left" style="width: 100%;">
						<div class="img">
    ...
    @include('partials.account.painter_chats')
					...
					{{--<div class="acting-item clearfix w-100">--}}
...
@endforeach
    ...
    @if (!is_array($orders))
        <div class="pagination pagination_orders">
            {{ $orders->links() }}
        </div>
    @endif
