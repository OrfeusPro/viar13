        ...
        @foreach($dataRow->details->extra_fields as $key => $field)
            ...
            @endif
                @if($field->type === 'dropdown')
                    @php
                        $data = \App\Models\GalleryItem::where('id',$model['id'])->first()->royal_category;
                        $list = explode("\n",$data);
                    @endphp
                    <div class="w-modal-form-group @if(isset($field->class)) {{ $field->class }} @endif">
                        <label class="w-modal-label" for="acebox-{{ $key }}">{{ $field->title }}</label>
                        <select  class="w-modal-form-control" name="{{ $key }}"
                                 placeholder="{{ $field->title }}">
                            @foreach($list as $option)
                                <option @if(trim($option) == $file->getCustomProperty($key)) selected @endif value="{{trim($option)}}">{{trans('portrait_royal.'.trim($option))}}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
