@php
    ...
    $custom_labels_config = [
        "1056_custom_size_prices" => "Цены холст",
        "1056_custom_size_prices_form2" => "Цены бумага",
...
{{--                                    @php if (Str::slug($row->details->tab_title)=="tsenyrazmery") {continue;} @endphp--}}
                                ...
                                <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                    {{ $row->slugify }}
                                    @if(isset($custom_labels_config[$dataTypeContent->id."_".$row->field]))
                                        <label class="control-label" for="name">{{$custom_labels_config[$dataTypeContent->id."_".$row->field]}}</label>
                                    @else
...
@section('javascript')
    ...
    @include('voyager-extension::master.templates');
    @include('voyager-extension::master.js');
@stop
