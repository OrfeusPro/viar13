    ...
    <div class="page-content">
        <table id="dataTable" class="table table-hover dataTable no-footer sortable" role="grid" aria-describedby="dataTable_info">
            ...
            @foreach($users as $user)
                ...
                <td> {{ $user->email }}</td>
                    <td> <a href="{{ $user->screenshot }}"><img style="max-width: 200px;" src="{{ $user->screenshot }}"></a></td>

{{--  select/options with $user->is_facebook_sake options = 0 or 1 with default option depend on $user->is_facebook_sake  --}}
                    <td>
                        <select class="is_facebook_sale" data-id="{{ $user->id }}" name="facebooksale">
                            <option value='0' {{ $user->is_facebook_sale == '0' ? 'selected' : '' }}>Нет</option>
                            <option value='1' {{ $user->is_facebook_sale == '1' ? 'selected' : '' }}>Да</option>
                            <option  {{ $user->is_facebook_sale === NULL ? 'selected' : '' }} disabled >Не проверено</option>
                        </select>
                    </td>
...
@stop
<script>
    $(document).on('change', '.is_facebook_sale', function(e){
        var sale = $(this).val();
        ...
        $.ajax({
            ...
            url: '/admin/user/'+user_id+'/set_sale/1',
            data: form_data,
