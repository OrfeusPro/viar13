...
- [TODO] Клиент/художник/SA: вернуть ответ рядом с историей; сейчас отдельный
  Filament modal добавляет шаг и скрывает контекст, внутренний чат уже inline.
- [TODO] Убрать тупиковые варианты «По наброску/По картине» при отсутствии
  соответствующих изображений (№18451: пустой обязательный select).
- [TODO] Сверить автообновление SA: legacy содержит polling каждые 5 секунд,
  Filament history пока загружается при открытии; без новых сообщений в этом
  read-only аудите сквозной приход ещё не подтверждён.
- [TODO] Вернуть безопасные кликабельные HTTP(S)-ссылки в общем клиентском
  чате: legacy Blade делает linkify, новый выводит простой текст. Найдено
  по коду; на пустой истории №18451 browser-подтверждение отсутствует.
...
- [TODO] Уточнить сообщение формы художника при отсутствии назначения:
  общий чат доступен, но обещание видимости «назначенному художнику» неточно.
