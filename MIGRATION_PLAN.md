...
- [~] [IN PROGRESS] **ADM-FIL-003 — Список и фильтры заказов**: после прямого
  ...
  Следующий подблок этой же задачи — колонка «Оплата»: дата заказа, статус,
  предоплата и Venipak-этикетки переносятся строго по `payment.blade.php`;
  внешние payment email/webhook изолируются до завершения UAT.
  Включение внешней почты выполняется только отдельным шагом после приёмки.
