...
- [~] [IN PROGRESS] **ADM-FIL-003 — Список и фильтры заказов**: после прямого
  ...
  Первый функциональный подблок «Номер» начат: изменение/замена/удаление
  VR00/BAW/VRR445/DS020 выполняется через permission и транзакционный сервис;
  фирма, счёт и payment-request actions ещё остаются TODO этой же задачи.
