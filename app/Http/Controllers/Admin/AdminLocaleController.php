...
class AdminLocaleController extends Controller
{
    ...
    public function ajax_admin_get_lng_template(Request $request)
    {
        $lng = $request->input('lng');
        $response = [];

        $templates = scandir('../resources/lang/' . $lng);
        unset($templates[0]);
        unset($templates[1]);

        foreach ($templates as $template) {
            $response[] = [
                'name' => explode('.', $template)[0],
                'template' => include('../resources/lang/' . $lng . '/' . $template)
            ];
        }
    ...
    public function ajax_admin_set_lng_template(Request $request)
    {
        ...
        $lvl = count(explode(':', $key));

        $lines = include('../resources/lang/' . $prefix . '/' . $template . '.php');
        ...
        switch (count($keys)) {
            case 1:
                ...
                break;
            case 2:
                $lines[$keys[0]][$keys[1]] = $value;
                break;
            case 3:
                $lines[$keys[0]][$keys[1]][$keys[2]] = $value;
                break;
        }

        $output = json_decode(str_replace(['(', ')'], ['&#40', '&#41'], json_encode($lines)), true);
        ...
        $fd = fopen('../resources/lang/' . $prefix . '/' . $template . '.php', 'w');
        fwrite($fd, $str);
        fclose($fd);
    }

    public function template(Locale $locale)
    {
        $locales = $locale->getLocales();

        return view('admin.locale_template', ["locales" => $locales]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            ...
            if (!file_exists('../resources/lang/' . $prefix)) {
                mkdir('../resources/lang/' . $prefix, 0777, true);
            }
