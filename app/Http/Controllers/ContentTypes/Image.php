...
class Image extends BaseType
{
    public function handle()
    {
        if ($this->request->hasFile($this->row->field)) {
            ...
            $this->generateWebpForUpload($fullPath, $file->getClientOriginalExtension());
    ...
    /**
     * Create a WebP file next to the stored original when possible.
     *
     * @param string $fullPath
     * @param string $extension
     * @return void
     */
    protected function generateWebpForUpload(
        string $fullPath,
        string $extension
    ): void {
        ...
        $supportedExtensions = function_exists('\\image_webp_supported_extensions')
        ...
        try {
            \image_webp_url('storage/' . ltrim(str_replace('\\', '/', $fullPath), '/'), true);
        } catch (\Throwable $exception) {
            // Upload must not fail if WebP generation is unavailable on the server.
        }
    }
    ...
    private function is_animated_gif(
        $filename
    ) {
