document.addEventListener("DOMContentLoaded", function (event) {
  ...
  $(document).on("submit", ".js_forget_pass_new", function (e) {
    ...
    const locale = $(this).find('input[name="locale"]').val();
    $.ajax({
      method: "POST",
      headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content") },
      url: route_forget,
      data: { email: user_email, locale: locale },
  ...
  if (header) {
    ...
    const syncHeader = () => {
      ...
      header.classList.toggle("is-fixed", isFixed);

      const bounds = header.getBoundingClientRect();
      root.style.setProperty("--header-offset", `${Math.round(bounds.bottom)}px`);
    };
