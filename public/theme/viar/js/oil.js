$(function () {
    ...
    var $allPortraitItems = $('.portrait-list__slider .portrait-list__item').clone(true);

    // 2) Выносим init в функцию
    ...
    // 3) Инициализируем слайдер один раз
    initPortraitSlider();
  
    // 4) Переписываем click-handler для табов
    $(document).on("click", ".tabs-js", function () {
      var $tab = $(this),
          cat  = $tab.data("cat"); // undefined означает «Все»
  
      // переключаем active
      $(".tabs-js").removeClass("active");
      $tab.addClass("active");
  
      // полностью «разбиваем» старый слайдер и очищаем HTML
      $(".portrait-list__slider")
        .slick("unslick")
        .empty();
  
      // наполняем контейнер нужными элементами
      if (cat) {
        // только те, у кого data-cat = выбранная
        $allPortraitItems
          .filter('[data-cat="'+cat+'"]')
          .appendTo(".portrait-list__slider");
      } else {
        // все
        $allPortraitItems.appendTo(".portrait-list__slider");
      }
  
      // инициализируем заново
      initPortraitSlider();
  
      // ваш centerLI, если нужен
