...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '.cart-delivery-point__pickup-point--item', function (e) {
    ...
    let _this = this;
    setTimeout(function () {
      let text = $(_this).find('span').html().split("<br>");
      text = text[0].trim() + ' ' + text[1].trim();
      $('.cart-data-page__input.point input').val(text.trim());
      $('.cart-delivery-point__pickup-point--wrapper').fadeOut();
      $('.wind-open').fadeIn();
    }, 500)
  });

  $(document).on('click', '.wind-open', function (e) {
    e.preventDefault();
    $('.cart-delivery-point__pickup-point--wrapper').fadeIn();
  ...
  $('.header-new-drop__item').hover(function (e) {
    ...
    if (!img) return;
    let preview = $(this).parents('.header-new-drop__list').siblings('.header-new-drop__img').find('.js-img-drop');
    let attributes = $(this).attr('data-image-alt-attributes');
    preview.attr('src', img);
    if (attributes !== undefined) {
      let source = $('<img ' + attributes + '>');
      preview.attr('alt', source.attr('alt') || '');
      if (source.attr('title')) preview.attr('title', source.attr('title'));
      else preview.removeAttr('title');
    }
  });
...
for (const dropdown of document.querySelectorAll('.select__options')) {
  dropdown.addEventListener('click', function (e) {
    ...
    // document.body.style.overflow = document.body.style.overflow == 'hidden' ? 'visible' : 'hidden';
    document.querySelector('.cart-data-page__input.point input').value = '';
    $('.cart-delivery-point__pickup-point--wrapper').fadeIn();
  });
}
...
$(document).on('click', '.city .select__option', function(e) {
  e.preventDefault();
  $('.city .select__option').removeClass('selected');
  $(this).addClass('selected');
  $('.all-city-cart').text($(this).data('value'))
})
// select-default
