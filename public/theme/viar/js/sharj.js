...
// This entry need to be wrapped in an IIFE because it need to be isolated against other modules in the chunk.
  ...
  \***************************************/
        ...
        $(function ()
        {
           // var size_price = $('.js_sizes.active').find('.kviz-radio_active').find('input:not(:disabled)').first().data('price');
            ...
            var obraz_img = "";
            var obraz_title = "";
            var getActiveSetInput = function getActiveSetInput() {
                return $('.set__item.kviz-radio_active').find('input').first().add($('.js_set_item.kviz-radio_active').find('input').first()).add($('input.js_set:checked').first()).add($('input[name="boxes[]"]:checked').first()).filter(function () {
                    return $(this).length && typeof $(this).val() !== 'undefined';
                }).first();
            };

            var calc_new_total_price = function calc_new_total_price() {
                var setInput = getActiveSetInput();
                set_price = setInput.length ? setInput.val() : 0;
                // console.log('----------------------------------------------------------------------------------------------------------------------');
                //
                // console.log('Массив цен',size_price);
                //
                if (window.template && window.template['obraz'] && typeof window.template['obraz'] === 'string' && window.template['obraz'].trim() !== '') {
                    type=$('#obraz input[name="type"]:checked').val();
                    size_price=$('#obraz').find('.size_for__' + type).find('.kviz-radio_active').data('price');
                } else {
                    if (type==undefined) {
                        type=$('#sharj input[name="type"]:checked').val();
                    }
                    type=$('#sharj input[name="type"]:checked').val();
                    size_price=$('#sharj').find('.size_for__' + type).find('.kviz-radio_active').data('price');
                }

               // size_price = $('.js_sizes.active').find('.kviz-radio_active').data('price');

                if (canvas_price === null || isNaN(parseFloat(canvas_price)) || parseFloat(canvas_price)==0) {
                    canvas_price=1;
                }
                // console.log("size_price",size_price);

                var totalPrice = parseFloat(size_price)*parseFloat(canvas_price) + parseInt(frame_price) + parseFloat(set_price) + parseFloat(persons_price) + parseFloat(decor_price) + parseFloat(time_price);

                // console.log("sharj1",canvas_price);
                // console.log("totalPrice",totalPrice);
                 //console.log("size_price",size_price);
                // console.log("canvas_price",canvas_price);
                // console.log("frame_price",frame_price);
                // console.log("set_price",set_price);
                // console.log("persons_price",persons_price);
                // console.log("decor_price",decor_price);
                // console.log("time_price",time_price);
                // console.log(size_price, totalPrice);

                if (!isNaN(totalPrice)) {
                    total_price = totalPrice.toFixed(2);
                    // console.log("total_price1",total_price);
                    $('.js_price').text(total_price);
                } else {
                    total_price = totalPrice.toFixed(2);
            ...
            $(document).on('click', '.form-image', function (e) {
                ...
                var closestJsSizes = $(this).closest('.js_sizes.active');
                size_price = closestJsSizes.find('.kviz-radio_active').find('input:not(:disabled)').first().data('price');
                calc_new_total_price();
            ...
            $(document).on('click', '.frame-item', function (e) {
                frame_price = $(e.currentTarget).data('price');
                calc_new_total_price();
            })
            ...
            $(document).on('change', 'input[name="boxes[]"]', function (e) {
                calc_new_total_price();
            });

            $(document).on('change', '.js_plot_type input', function (e) {

                var $dom = $(e.currentTarget);
                let parentsContainer = $(this).closest('.formalization-items');
                var type = $dom.val();
                var form = $(this).closest('form');

                parentsContainer.find('.frame_for').hide();
                parentsContainer.find('.frame_for__' + type).show();
                parentsContainer.find('.frame_for').find('.frame-item').removeClass('active');
                frame_price = 0;
                parentsContainer.find('.size_for').removeClass('active');
                parentsContainer.find('.size_for').hide();
                parentsContainer.find('.size_for__' + type).show();
                parentsContainer.find('.size_for__' + type).addClass("active");;

               // size_price = parentsContainer.find('.size_for__' + type).find('.kviz-radio_active').data('price');

                if (window.template['obraz']!=null||window.template['obraz']!=undefined){
                    size_price=form.find('#tempplate_sizes').find('.size_for__' + type).find('.kviz-radio_active').data('price');
                } else  {
                    size_price=form.find('#main_sizes').find('.size_for__' + type).find('.kviz-radio_active').data('price');
                }
            ...
            $(document).on('click', '.js_submit_form', function (e) {
                console.log("Нажатие на добавить в корзину");
                e.preventDefault();
                obj=$(this);
                ...
                if (window.template != undefined && window.template['fon'] != undefined && window.template['fon'] != null) {
                    data.append('fon', window.template['fon']);
                }
                if (window.template != undefined && window.template['obraz'] != undefined && window.template['obraz'] != null) {
                    data.append('obraz', window.template['obraz']);
                }




                if (data.has('obraz') || data.has('fon') ) {
                    //data.append('size', $('#obraz').find('.js_sizes:not(.active)').find('.kviz-radio_active').find('input').val());
                    data.append('full_size', $('#obraz').find('.js_sizes.active').find('.kviz-radio_active').find('input').data('full-size') || $('#obraz').find('.js_sizes.active').find('.kviz-radio_active').find('input').val());
                    data.append('size', $('#obraz').find('.js_sizes.active').find('.kviz-radio_active').find('input').val());
                } else
                {
                    data.append('full_size', $('#sharj').find('.js_sizes.active').find('.kviz-radio_active').find('input').data('full-size') || $('#sharj').find('.js_sizes.active').find('.kviz-radio_active').find('input').val());
                    data.append('size', $('#sharj').find('.js_sizes.active').find('.kviz-radio_active').find('input').val());
                }
                ...
                if (window.loadedImages) {
                    ...
                    for (var x = 0; x < loadedImage.length; x++) {
                        if (loadedImage[x]) {
                            data.append('orig_images[]', loadedImage[x]);
                            console.log(loadedImage[x]);
                        }
                    }
                } else {
                ...
                // var sizeValue = $('.js_sizes.active').find('.kviz-radio_active').find('input:not(:disabled)').first();
                //
                ...
                var form = $(this).closest('form');
                var plotTypeLabel = form.find('.js_plot_type .kviz-radio_active label');
                var plotTypeText = plotTypeLabel.text();
                data.append('type', plotTypeText);

              // data.append('type', $('.js_plot_type .kviz-radio_active label').text()); //fix
                ...
                data.append('ram_id', $('.frame-item.active').data('code'));
                data.append('userComment', $('#comments1').val());
                data.append("obraz_title", $('input[name="obraz_title"]').val());
                data.append("obraz_img", $('input[name="obraz_img"]').val());
                ...
                var activeSetInput = getActiveSetInput();
                data.append('compl_id', activeSetInput.length ? activeSetInput.data('id') : '');

                var termsSelected = form.find('.js_terms.kviz-radio_active .js_term__selected');

                var termsText = termsSelected.text();

                var termsPriceValue = termsSelected.next().val();

                data.append('terms', termsText);
                data.append('terms_price', termsPriceValue);
            ...
            $(document).on("click", ".js-simps-calc", function (e) {
                console.log("Нажатие на кнопку попапа");
                window.template = [];
                e.preventDefault();

                var form = $(this).closest('form');

                if ($(this).attr('attr_image') !== undefined) {
                    title = $(this).attr('attr_name');
                    image = $(this).attr('attr_image');
                    $('.popup-dyn-image').attr('src', image);
                    stype=$(this).attr('attr_type');
                    $('input[name=obraz_img]').val(image);
                    // $('input[name=obraz_title]').val(title);
                  // $(".popup-wrapper .simpson-bg-selector").hide();

                    if (stype=='fon') {
                        $('.js_submit_form').attr('sbg', title);
                        $('.popup-wrapper-template .formalization__subrow').css('display','flex');
                        window.template['fon']=title;

                    }
                    if (stype=='obraz')  {
                        $('.js_submit_form').attr('obg', title);
                        $('.popup-wrapper-template .formalization__subrow').css('display','flex');

                        window.template['obraz']=title;
                        showNewPopupTemplate();
                    }
                }
                else
                {
                    $('.popup-wrapper-template .formalization__subrow').css('display','none');
                    window.template['fon']=null;
                    window.template['obraz']=null;
                    showNewPopup();
                }
               ...
               // $(".simpson-calculator").show();
                ...
                // size_price = $('.js_sizes.active').find('.kviz-radio_active').find('input:not(:disabled)').first().data('price');
                // console.log('Образ',window.template['obraz']);
                // console.log("Click on template popup");
                // console.log("Click on template popup",size_price);
            ...
            $(document).on("click", ".select-simpsons-bg", function (e) {
                e.preventDefault();
                image = $(this).attr('src');
                title = $(this).attr('attr_name');
                ...
                $('.popup-dyn-image').attr('src', image);
                $('.popup-wrapper .formalization__subrow').show();
            ...
            //     if (window.template['obraz']!=null||window.template['obraz']!=undefined){
            //         $(".popup-wrapper").addClass("obraz_form");
            //     } else {
            //         $(".popup-wrapper").addClass("main_form");
            //     }
            //     $("body").addClass("fixed");
            ...
            if (window.matchMedia("(min-width: 768px)").matches) {
                ...
                function setPadding() {
                    let padding =
                        parseInt($(".section-frame:not(.vz-art)").css("margin-left")) + 20;
            ...
            document.querySelectorAll(".sectionDefault-tabs__item").forEach((element) => {
                element.addEventListener("click", function (e) {
                    e.preventDefault();
                    let _this = this;
                    changeActiveTab(
      ...
      `
            ...
            function clear_image_popup(element)
            {
                console.log(element);
                console.log(_input);

                const inputElement = $("#image-input");
                inputElement.val("");
                window.loadedImages = [];
                $(".images-container").html("");
            ...
            function showNewPopup() {
                $("body").addClass("fixed");
                $(".popup-wrapper").addClass("active");
                element=$(".popup-wrapper").find(".abs-close");
                _input=$(".popup-wrapper").find(".file-input");
                clear_image_popup(element,_input);
                calc_new_total_price();
            }

            function showNewPopupTemplate() {
                $("body").addClass("fixed");
                $(".popup-wrapper-template").addClass("active");
                element=$(".popup-wrapper-template").find(".abs-close");
                _input=$(".popup-wrapper-template").find(".file-input");
                clear_image_popup(element,_input);
                window.loadedImages =[];
                calc_new_total_price();
            }
            ...
            function closeNewPopup() {
                $("body").removeClass("fixed");
                $(".popup-wrapper").removeClass("active");
                $(".popup-wrapper-template").removeClass("active");
                window.template['fon']=null;
                window.template['obraz']=null;
                element=$(".popup-wrapper").find(".abs-close");
                clear_image_popup(element,_input);
            ...
            $(document).on('click', '.portrait-list__item img', function() {
                $('.portrait-list__item img').removeClass('active');
                $(this).addClass('active');
            })
            ...
            $('.top-all').click(function (e) {
                ...
                var more = $('.js_more_wks').data('more');
                var hide = $('.js_more_wks').data('less');
                ...
                if (topHideItem) {
                    ...
                    topFade += 4;
                    $('.top-item_hide:lt(' + topFade + ')').css("display", "flex")
                        .hide()
                        .fadeIn();
                    topHideItem = false;
