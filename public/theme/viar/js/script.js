...
$(document).ready(function () {
    ...
    function reviewImagesOrientationAction() {
		var imgs = document.querySelectorAll(".reviews-photo"),
		  len = imgs.length,
		  counter = 0;
		...
		//   if (counter === len) {
			$(imgs).each(function () {
			  let img = this;
			  let _imgNaturalWidth = img.naturalWidth;
			  let _imgNaturalHeight = img.naturalHeight;
			  if (_imgNaturalWidth / _imgNaturalHeight > 1.2) {
				$(img).closest(".reviews-slide").addClass("orientation-changed");
			...
			var hi = 0;
			setTimeout(function() {
			  $(".reviews-slide-box").each(function(){
				var h = $(this).height();
				if(h > hi){
					hi = h;
					highest = h;
	  ...
	  if ($(".reviews-photo").length) {
		  reviewImagesOrientationAction();

		  $('.reviews-slide audio').each(function() {
	...
	let default_src = $('.stock-photo').attr('src');

	$('.kviz-group .kviz-radio').on('click', function (e) {
		e.preventDefault();

		let src = $(this).find('input').data('img');
		if ((/\.(gif|jpe?g|tiff?|png|webp|bmp)$/i).test(src)) {
			$('.stock-full .stock-photo').attr('src', src);
			$('.stock-full .stock-photo').attr('srcset', src);
			$('.stock-full picture source').attr('srcset', src);
		} else {
			$('.stock-full .stock-photo').attr('src', default_src);
			$('.stock-full .stock-photo').attr('srcset', default_src);
			$('.stock-full picture source').attr('srcset', default_src);
		}
	...
	$('.submit-form, #generate form').on('submit', function() {
		$(this).find('.loading-bar').css("display", "flex")
		.hide()
		.fadeIn();
	})

	$(document).on('change', '.canvas_select_sizes, .select-size', function() {
		var $selectedOption = $(this).find('option:selected');
		var priceLabel = $selectedOption.attr('data-price-label') || '';
		$(this).closest('form').find('.js-price-label-field').val(priceLabel);
	});

	$('.canvas_select_sizes, .select-size').each(function() {
		var $selectedOption = $(this).find('option:selected');
		var priceLabel = $selectedOption.attr('data-price-label') || '';
		$(this).closest('form').find('.js-price-label-field').val(priceLabel);
	});
	...
	$('.burger-close').click(function (e) {
		e.preventDefault();
		$('.burge-menu').fadeOut();
		$('.vz-header').removeClass('is-hidden');
		$('.menu').fadeOut();
...
if (!$('body').hasClass('vz-art')){
        ...
        $('.js-popup-photo').click(function (e) {
            ...
            $('body').addClass('open-frame');
            $('.popup-frame').css("display", "flex").hide().fadeIn();
            let style = $(this).data('style').replace(/<[^>]*>?/gm, '');
             $(".popup-photo-header .select-style")
                .find('option')
                .remove()
                .end()
                .append(`<option value="${style}">${style}</option>`)
                .val(style);
            // $(".popup-photo-header .select-style option[value='" + style + "']").prop("selected", "selected");
	...
	// $('body.vz-home .js-examples').click(function (e) {
	// 	e.preventDefault();
	...
	// 	$(".popup-photo .select-style option[value='" + style + "']").prop("selected", "selected");
	// 	$(".popup-photo .select-size option[value='" + size + "']").prop("selected", "selected");
	// });


	// $('body.vz-art .js-examples').click(function (e) {
	// 	e.preventDefault();

	// 	let catId = $(this).data('catid');
	// 	// let style = $('.js-popup-photo').data('style')
	// 	// let style_mod = style.replace(/<[^>]*>?/gm, '');

	// 	// $('body').addClass('open-frame');
	// 	// $('.popup-frame').css("display", "flex").hide().fadeIn();
	// 	// $('.popup-photo').fadeIn();

	// 	// $(".popup-photo .select-style")
    //     //         .find('option')
    //     //         .remove()
    //     //         .end()
    //     //         .append(`<option value="${style_mod}">${style_mod}</option>`)
    //     //         .val(style_mod);

	// 	$(".popup-photo .select-style option[datacatid='" + catId + "']").prop("selected", "selected");
	// 	// $(".popup-photo .select-size option[value='" + size + "']").prop("selected", "selected");
	// });

	$('.js-cl-works').click(function (e) {
	    e.preventDefault();
        $('.js-popup-photo').trigger('click');
    });

	$('.js-works-examples').click(function (e) {
		e.preventDefault();
		let size = $(this).data('size');
		// let style = $(this).data('style');
		$('body').addClass('open-frame');

		let style = $('.js-popup-photo').data('style').replace(/<[^>]*>?/gm, '');

		$(".popup-works-ex .select-style")
                .find('option')
                .remove()
                .end()
                .append(`<option value="${style}">${style}</option>`)
		...
		$('.popup-frame').css("display", "flex").hide().fadeIn();
		$('.popup-works-ex').fadeIn();

		// $(".popup-works-ex .select-style option[value='" + style + "']").prop("selected", "selected");
		$(".popup-works-ex .select-size option[value='" + size + "']").prop("selected", "selected");
	});
	...
	$('.js-popup-login').click(function (e) {
		e.preventDefault();
		$('body').addClass('open-frame');
		$('.popup-frame:not(.popup-frame-alternative-size):not(.popup-frame-recommendation)').css("display", "flex").hide().fadeIn();
		$('.popup-login').fadeIn();
	});
...
$(".input-file_wrapper input").on("change", function () {
    let input = $(this);
    var reader = new FileReader();
    console.log(reader)
    let container = $(this).closest(".input-file_wrapper").find('.r-files_box');
	$(this).closest('.input-file_wrapper').addClass('active-input');
...
function checkRegWithCoupon () {
    const isHomepage = window.location.pathname === '/';
    const hasHash = window.location.hash === '#reg';
