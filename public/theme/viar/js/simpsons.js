...
// This entry need to be wrapped in an IIFE because it need to be isolated against other modules in the chunk.
  ...
  \***************************************/
        ...
        $(function () {
            ...
            var time_price = 0;
            var total_price = 0;
            var getActiveSetInput = function getActiveSetInput(form) {
                return form.find('.set__item.kviz-radio_active input').first().add(form.find('.js_set_item.kviz-radio_active input').first()).add(form.find('input.js_set:checked').first()).add(form.find('input[name="boxes[]"]:checked').first()).filter(function () {
                    return $(this).length && typeof $(this).val() !== 'undefined';
                }).first();
            };

            var calc_new_total_price = function calc_new_total_price() {
                var setInput = getActiveSetInput($(document));
                set_price = setInput.length ? setInput.val() : 0;
                if (canvas_price === null || isNaN(parseFloat(canvas_price)) || parseFloat(canvas_price)==0) {
                    canvas_price=1;
                }
                console.log("simpsons",canvas_price);
            ...
            $(document).on('click', '.js_submit_form', function (e) {
                ...
                let form = $(this).closest('form'); // Находим родительскую форму кнопки
                ...
                data.append('size', form.find('.js_sizes.active .kviz-radio_active input').val());
                data.append('users_count', users_count); //fix

                data.append('type', form.find('.js_plot_type .kviz-radio_active label').text()); //fix
                ...
                data.append('userComment', form.find('.order_comments').val());
                ...
                var activeSetInput = getActiveSetInput(form);
                data.append('compl_id', activeSetInput.length ? activeSetInput.data('id') : '');
                data.append('terms', form.find('.js_terms.kviz-radio_active .js_term__selected').text());
                data.append('terms_price', form.find('.js_terms.kviz-radio_active .js_term__selected').next().val());
    ...
    $(document).on("click", ".js-simps-calc", function (e) {
        ...
        if ($(this).attr('attr_image') !== undefined) {
            ...
            if (stype=='fon') {
                ...
                $('.popup-wrapper .formalization__subrow').css('display','flex');
            }
            if (stype=='obraz')  {
                $('.js_submit_form').attr('obg', title);
                // $(".popup-wrapper .simpson-bg-selector").show();
                $('.popup-wrapper .formalization__subrow').css('display','flex');
            }

        } else
        {
            $('.popup-wrapper .formalization__subrow').css('display','none');
        }
...
$(document).on('click', '.portrait-list__img', function() {
    $('.portrait-list__img').removeClass('active');
    $(this).toggleClass('active');
})
