$(document).ready(function () {
    ...
    $(document).on('change', '.persons-item select', function(e) {
        var val = $(this).val(); //users
        var country_mp  = $('body').data('multiplier');
        if(country_mp == 1){
            country_mp = 0;
        }
        
        if(val == '#') {return;}
        var price = $(this).next('input').val() //size val

        console.log(val);
        console.log(price);
        
        var total = parseInt(val) + parseInt(price) + country_mp;
        $(this).closest('.item').find('.js__total').text(total);
    });

    // $(document).on('change', '.js_users_count', function(e) {
    //     var counts = $('.js_persons_all').data('persons');
    //     var counts_arr = counts.split(",");

    //     var val = $(this).val();
    //     var cat_data_price = $('.js_persons_all').data('var');
    //     var size_price = $(this).next('input').val();

    //     var total = parseInt(val) * parseInt(size_price);
    //     $(this).closest('.item').find('.js__total').text(total);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response) {
                    ...
                    if(response.message == 'The image must be a file of type: png, bmp, jpg, jpeg.'){
                        var msg = $('body').data('err_file');
                        alert(msg);
                    }
